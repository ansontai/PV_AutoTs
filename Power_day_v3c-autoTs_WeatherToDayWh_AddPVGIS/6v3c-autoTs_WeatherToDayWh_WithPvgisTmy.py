...
def main():
    ...
    try:
        echo_keys = ['default_max_generations', 'horizons', 'InfiniteLoop',
             ...
             'ENABLE_TMY', 'TMY_PATH', 'TMY_USE_COLUMNS']
    ...
    while True:
        ...
        for idx, horizon in enumerate(horizons):
            ...
            # start with TMY candidate if available
            pred_fr = pred_fr_candidate
            if ENABLE_FUTURE_REGRESSOR and ENABLE_PREDICT_FUTURE_REGRESSOR and FUTURE_REGRESSOR_PREDICT:
                try:
                    ...
                    if os.path.exists(p_path):
                        loaded_pred_fr = None
                        try:
                            loaded_pred_fr = pd.read_csv(p_path, parse_dates=['LocalTime'], index_col='LocalTime', low_memory=True)
                        except Exception:
                            loaded_pred_fr = pd.read_csv(p_path, low_memory=True)
                            if 'LocalTime' in loaded_pred_fr.columns:
                                loaded_pred_fr.index = pd.to_datetime(loaded_pred_fr['LocalTime'], errors='coerce')
                                loaded_pred_fr = loaded_pred_fr.drop(columns=['LocalTime'])
                        try:
                            loaded_pred_fr = loaded_pred_fr.reindex(test_df.index)
                        except Exception:
                            pass
                        # Drop target column if accidentally present
                        if loaded_pred_fr is not None and 'Wh' in loaded_pred_fr.columns:
                            loaded_pred_fr = loaded_pred_fr.drop(columns=['Wh'])
                        ...
                        try:
                            ...
                            non_allnan = [c for c in loaded_pred_fr.columns if not loaded_pred_fr[c].isna().all()]
                            if len(non_allnan) != len(loaded_pred_fr.columns):
                                dropped = set(loaded_pred_fr.columns) - set(non_allnan)
                                try:
                                    loaded_pred_fr = loaded_pred_fr[non_allnan]
                                except Exception:
                                    pass
                            ...
                            # If no regressors remain, do not include loaded_pred_fr
                            if loaded_pred_fr.shape[1] == 0:
                                print('No valid future regressors remain after cleaning; will not use loaded future_regressor')
                                loaded_pred_fr = None
                        except Exception as e_clean:
                            print('Warning: failed to clean/preserve future_regressor:', e_clean)
                        # merge loaded_pred_fr with existing pred_fr (e.g., TMY candidate)
                        if loaded_pred_fr is not None:
                            pred_fr = _merge_fr(pred_fr, loaded_pred_fr)
                            print('Loaded and merged future regressor for predict from', p_path)
                        else:
                            print('No valid future regressor columns found in', p_path)
                    else:
