...
def run_horizon(AutoTS, ser, horizon, output_dir, timestamp, count, n_jobs, max_generations,
    ...
    def _run():
        ...
        # Accept AutoTS native 'auto' or clamp explicit integer validations for short series.
        if isinstance(num_validations, str) and num_validations.strip().lower() == 'auto':
            autots_num_validations = 'auto'
        else:
            requested_nv = max(1, int(num_validations))
            autots_num_validations = min(requested_nv, max(1, len(dfp_train) // 2))
        print(f'Using num_validations={autots_num_validations} for horizon={horizon}')
