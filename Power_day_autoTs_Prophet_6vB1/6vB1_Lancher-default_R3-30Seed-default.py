...
DEFAULT_OUTPUT_TAG = 'default_AllSettings'
        ...
        total = len(seeds)
                ...
                try:
                    with open(logp, 'a', encoding='utf-8') as lf:
                        lf.write(f"{datetime.now().isoformat()} - seed {i}/{total} - cmd: {' '.join(cmd)} - exit: {getattr(e, 'returncode', 'unknown')}\n")
                        lf.write(str(e) + '\n')
                except Exception:
                    pass
...
")
            ...
            try:
                with open(logp, 'a', encoding='utf-8') as lf:
                    lf.write(f"{datetime.now().isoformat()} - single run - cmd: {' '.join(cmd)} - exit: {getattr(e, 'returncode', 'unknown')}\n")
                    lf.write(str(e) + '\n')
            except Exception:
                pass
