...
def run_horizon(AutoTS, ser, horizon, output_dir, timestamp, count, n_jobs, max_generations, transformer_list, model_list, ensemble, metric_weighting=None):
    ...
    def _run():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                if 'horizon_dir' in locals():
                    os.makedirs(horizon_dir, exist_ok=True)
                    with open(os.path.join(horizon_dir, 'autots_fail.log'), 'a', encoding='utf-8') as lf:
    ...
    try:
        ...
        try:
            ...
            # try writing into horizon_dir, then output_dir, then script dir, then temp dir
            write_paths = [os.path.join(output_dir, str(horizon)), output_dir, os.path.dirname(__file__), tempfile.gettempdir()]
            written = False
...
def main():
    ...
    if getattr(args, 'debug', False):
        debug_log_path = args.debug_log or os.path.join(os.path.dirname(__file__), 'child_debug.log')
        try:
