    ...
    def _run():
        ...
        try:
            ...
            if len(y_pred) == len(y_true):
                ...
                try:
                    plot_path_lv2 = os.path.join(horizon_dir, f'forecast_vs_actual_vs_lastvalue_{horizon}-format2.png')
                    plot_forecast_comparison_legacy(plot_path_lv2, test_ser.index, y_true, y_pred, y_lastvalue,
                                                    mase=scores.get('MASE_lastValue'),
                                                    rmsse=scores.get('RMSSE'),
                                                    smape=scores.get('SMAPE(%)'),
                                                    title=f'Wh Forecast vs Actual vs LastValue ({horizon}d) - format2',
                                                    baseline_label='LastValue')
                    print('Saved LastValue comparison chart (format2) to', plot_path_lv2)
                except Exception as e:
                    print('Failed to save LastValue comparison chart format2:', e)
            else:
