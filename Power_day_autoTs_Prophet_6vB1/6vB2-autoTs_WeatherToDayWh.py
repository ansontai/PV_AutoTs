...
def run_horizon(AutoTS, ser, horizon, output_dir, timestamp, count, n_jobs, max_generations,
    ...
    def _run():
        ...
        # Cap requested validations by available training length to avoid out-of-bounds.
        requested_nv = max(1, int(num_validations))
        desired_nv = min(requested_nv, max(1, len(dfp_train) // 2))
...
def main():
    ...
    parser.add_argument('--num_validations', type=int, default=5, help='AutoTS num_validations')
