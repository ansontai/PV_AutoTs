...
# default_transformer_list = []
# default_transformer_list = 'superfast' # AutoTS 內建的超快速測試組合（包含 DifferencedTransformer 和 Scaler）
default_transformer_list = 'fast' # AutoTS 內建的快速測試組合（包含 DifferencedTransformer、Scaler、DatepartRegression）
    ...
    while True:
        ...
        for idx, horizon in enumerate(horizons):
            ...
            ats_kwargs = dict(
                ...
                no_negatives=True,
                # Only pass random_seed if it is not None; some launcher configs
                # set random_seed=None which causes AutoTS internals to call
                # int(None) and raise a TypeError.
                # We'll include it conditionally below.
                )
            ...
            # basic sanity checks to avoid AutoTS producing all-NA validation metrics
            # coerce to numeric and drop any columns that are entirely NA
            try:
            ...
            if train_wide.empty or train_wide.isna().all().all():
                print('Skipping horizon: train_wide empty or all-NA after cleaning')
                continue
            ...
            fit_fr = None
            if ENABLE_FUTURE_REGRESSOR and ENABLE_FIT_FUTURE_REGRESSOR and FUTURE_REGRESSOR_TRAIN:
                try:
                ...
                if fit_fr is not None:
                    try:
                        ...
                        # Drop regressors that are entirely NaN (they cause AutoTS to produce all-NA validation results)
                        try:
                            allnan_cols = [c for c in fit_fr.columns if fit_fr[c].isna().all()]
                            if allnan_cols:
                                try:
                                    fit_fr = fit_fr.drop(columns=allnan_cols)
                                except Exception:
                                    pass
                                print('Dropped all-NaN columns from fit_fr:', allnan_cols)
                            if fit_fr.shape[1] == 0:
                                print('fit_fr has no valid columns after cleaning; disabling future_regressor for fit')
                                fit_fr = None
                        except Exception:
                            pass
                    except Exception as e:
            ...
            # Adjust num_validations for very short training series to avoid empty validation splits
            try:
                desired_nv = min(default_num_validations, max(1, len(train_wide) // 2))
                try:
                    # Some AutoTS versions read this attribute; set if available
            ...
            try:
                if fit_fr is not None:
                    ...
                    except ValueError as e_fit_fr:
                        msg_fit = str(e_fit_fr).lower()
                        print('AutoTS.fit ValueError (with future_regressor):', msg_fit)
                        if 'encountered all na values' in msg_fit or 'encountered all na' in msg_fit:
                            print('AutoTS.fit produced all-NA validation metrics; skipping this horizon')
                            # skip this horizon by jumping to the next iteration of horizons loop
                            continue
                        else:
                            # re-raise to be handled by outer handler below
                            raise
                else:
            ...
            # Diagnostic: print model results and key attributes before predict
            print('Diagnostic: model summary before predict')
            try:
                print('Model type:', type(model))
            except Exception:
                pass
            try:
                print('Model init params (get_params):')
                gp = getattr(model, 'get_params', None)
                if callable(gp):
                    try:
                        print(gp())
                    except Exception:
                        print('get_params() raised an exception')
                else:
                    print('get_params not available')
            except Exception:
                pass
            try:
                print('model.model_list attribute:', getattr(model, 'model_list', None))
            except Exception:
                pass
            try:
                print('model.transformer_list attribute:', getattr(model, 'transformer_list', None))
            except Exception:
                pass
            try:
                print('Best model id (best_model_id):', getattr(model, 'best_model_id', None))
            except Exception:
                pass
            try:
                print('Attempting to print model.results() summary:')
                mres = model.results()
                # If DataFrame-like, print head safely
                try:
                    if hasattr(mres, 'head'):
                        print(mres.head().to_string())
                    else:
                        print(mres)
                except Exception:
                    print('model.results() exists but failed to stringify')
            except Exception as e:
                print('Failed to call model.results():', e)
            ...
            try:
                if pred_fr is not None:
                    try:
                        ...
                        pass
                    # Additional diagnostics to help debug BestN / predict failures
                    try:
                        print('pred_fr.shape:', getattr(pred_fr, 'shape', None))
                        try:
                            print('pred_fr.columns:', list(pred_fr.columns))
                        except Exception:
                            pass
                        try:
                            na_counts = pred_fr.isna().sum()
                            print('pred_fr.isna().sum():\n', na_counts)
                        except Exception:
                            na_counts = None
                        try:
                            # Compare overlap with training regressors if available
                            tv_cols = getattr(train_value_df, 'columns', None)
                            if tv_cols is not None:
                                common = [c for c in pred_fr.columns if c in tv_cols]
                                print('Common regressors between pred_fr and train_value_df:', common)
                                if len(common) == 0:
                                    print('No common regressors between fit_fr and pred_fr; disabling predict future_regressor for safety')
                                    pred_fr = None
                        except Exception:
                            pass
                        try:
                            print('pred_fr length vs test_df length:', len(pred_fr), len(test_df))
                        except Exception:
                            pass
                        # Save diagnostics to CSV in out_dir for offline inspection
                        try:
                            sample_path = os.path.join(out_dir, 'pred_fr_sample.csv')
                            na_path = os.path.join(out_dir, 'pred_fr_na_counts.csv')
                            pred_fr.head(50).to_csv(sample_path, index=True)
                            if na_counts is not None:
                                na_counts.to_csv(na_path, index=True)
                            print('Saved pred_fr diagnostics to', sample_path, 'and', na_path)
                        except Exception as e_save_pf:
                            print('Failed to save pred_fr diagnostics:', e_save_pf)
                    except Exception:
                        pass
                ...
                # Special-case: BestN ensemble selection failed -> retry without future_regressor
                if 'bestn failed' in msg.lower() or 'bestn failed, no component models available' in msg.lower():
                    print('Detected BestN failure: saving model.results() and retrying predict without future_regressor (simple fallback)')
                    try:
                        try:
                            mres = model.results()
                            if hasattr(mres, 'to_csv'):
                                ...
                                print('model.results() returned non-DataFrame object; printing summary:\n', mres)
                            # Additional model.results diagnostics
                            try:
                                if hasattr(mres, 'query'):
                                    arima_rows = mres.query("Model == 'ARIMA'")
                                    print('ARIMA rows in model.results() (BestN failure path):')
                                    if hasattr(arima_rows, 'to_string'):
                                        print(arima_rows.to_string())
                                    else:
                                        print(arima_rows)
                                    if len(arima_rows) == 0:
                                        print('Warning: no ARIMA candidate in model.results()')
                            except Exception as e2:
                                print('Failed ARIMA candidate query on model.results():', e2)
                        except Exception as e_save:
                            print('Failed to call/save model.results():', e_save)

                        # Save traceback for the original exception to help debugging
                        try:
                            import traceback as _tb
                            trace_path = os.path.join(out_dir, 'predict_bestn_traceback.txt')
                            with open(trace_path, 'w', encoding='utf-8') as tf:
                                tf.write(_tb.format_exc())
                            print('Saved traceback to', trace_path)
                        except Exception as e_tb:
                            print('Failed to save traceback:', e_tb)

                        # First fallback: no future regressor
                        try:
                            print('Attempting fallback predict without future_regressor')
                            prediction = model.predict(verbose=3)
                        except Exception as e_no_fr:
                            print('Fallback no future_regressor also failed:', e_no_fr)
                            # Second fallback: force ARIMA-only, simple ensemble.
                            try:
                                print('Attempting ARIMA-only AutoTS re-fit and predict (model_list=["ARIMA"], ensemble=["simple"])')
                                arima_kwargs = dict(ats_kwargs)
                                arima_kwargs['model_list'] = ['ARIMA']
                                arima_kwargs['ensemble'] = ['simple']
                                # ensure no incorrect future_regressor options passed
                                fallback_model = AutoTS(**arima_kwargs)
                                fallback_model.forecast_length = horizon
                                # no future_regressor for fallback
                                fallback_model.fit(train_wide)
                        ...
                        print('BestN fallback chain failed (no_fr + arima-only):', e_retry)
                        raise
                elif 'invalid error value specified' in msg.lower():
                        ...
                        try:
                            ...
                            for c in p_conv.columns:
                                try:
                                    ...
                                    pass
                            print('Retrying model.predict with coerced future_regressor (verbose=3)')
                    ...
                    # If different ValueError, re-raise to surface the original problem
                    raise
