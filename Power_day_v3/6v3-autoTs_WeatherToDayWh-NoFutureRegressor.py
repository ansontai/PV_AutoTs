...
# To avoid BestN fails from unsupported candidate set, prefer simple ensemble for this no-future-regressor script.
...
# Note: future_regressor support removed in this variant — all related
# configuration and runtime branches have been removed to simplify usage.
...
def main():
    ...
    try:
        echo_keys = ['default_max_generations', 'horizons', 'InfiniteLoop',
             ...
             'default_transformer_list', 'default_num_validations', 'random_seed']
        ...
        for idx, horizon in enumerate(horizons):
            ...
            try:
                ...
                try:
                    ...
                    pass
            # no future_regressor support in this script; fit on training data only
            try:
            ...
            try:
                ...
                if 'invalid error value specified' in msg.lower():
                    print('Retrying model.fit without future_regressor (baseline)')
                    model = model.fit(train_wide)
                else:
                    raise
            ...
            # Simple prediction path without future_regressor support
            try:
            ...
            try:
                ...
                if 'bestn failed' in msg.lower() or 'bestn failed, no component models available' in msg.lower():
                    print('Detected BestN failure: retrying predict with ensemble="simple"')
                    try:
                        prediction = model.predict(ensemble='simple')
                    except Exception as e2:
                        print('Retry with ensemble="simple" failed:', e2)
                        print('Trying minimal model.predict() fallback')
                        try:
                            prediction = model.predict()
                        except Exception as e3:
                            print('Fallback model.predict() failed too:', e3)
                            # Last-resort fallback: strict ARIMA-only model
                            try:
                                print('Attempting ARIMA-only fallback model (ensemble=["simple"])')
                                arima_model = AutoTS(model_list=['ARIMA'], ensemble=['simple'], forecast_length=horizon, **safe_ats_kwargs)
                                arima_model.fit(train_wide)
                                prediction = arima_model.predict(verbose=3)
                                model = arima_model
                            except Exception as e4:
                                print('ARIMA-only fallback also failed:', e4)
                                raise
                else:
                    raise
