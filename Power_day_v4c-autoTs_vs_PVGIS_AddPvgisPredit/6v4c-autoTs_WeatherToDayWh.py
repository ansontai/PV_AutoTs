...
# --- PVGIS 作為外生變數的開關 (頂端常數控制) ---
# 設為 True 則會把 PVGIS daily scaled CSV 的 P_mapped_Wh 當作 AutoTS 的 future_regressor
ENABLE_PVGIS_AS_FUTURE_REGRESSOR = False
# PVGIS CSV 中預期的欄位名稱與回傳的前綴
PVGIS_FR_COLUMN = 'P_mapped_Wh'
PVGIS_FR_PREFIX = 'PVGIS'
...
def save_pvgis_vs_naive_metrics(train_df, test_df, out_dir, horizon, pvgis_csv_path=PVGIS_DAILY_SCALED):
    ...
    pvgis_metrics_path = os.path.join(out_dir, f'PVGIS_vs_naiveLag1_metrics_{horizon}d.json')
...
def load_pvgis_future_regressor(pvgis_csv_path, target_index, col_name=PVGIS_FR_COLUMN, prefix=PVGIS_FR_PREFIX):
    """Load PVGIS scaled series aligned to target_index and return a single-column DataFrame.

    The returned DataFrame is indexed by target_index and has a column named
    f"{prefix}_{col_name}" with numeric values (ffill/bfill filled).
    """
    try:
        ser = load_aligned_pvgis_series(target_index, pvgis_csv_path)
    except Exception:
        raise
    if ser is None:
        return None
    colname = f"{prefix}_{col_name}"
    df = ser.rename(colname).to_frame()
...
def plot_forecast_vs_actual_naive_vs_pvgis(
    ...
    """Save a chart comparing Actual / Forecast / Naive Lag-1 vs PVGIS."""
    ...
    plt.title(title or 'Wh Forecast vs Actual vs Naive Lag-1 vs PVGIS')
    plt.xlabel('Date')
    plt.ylabel('Wh')
    plt.grid(alpha=0.3)
    ax = plt.gca()
...
def plot_forecast_comparison_format2_pvgis(plot_path, index, y_true, y_pvgis, y_naive,
    ...
    plt.plot(index, y_naive, label='Naive Lag-1', color='gray', linewidth=2, linestyle='--')
    plt.title(title or 'Wh PVGIS vs Actual vs Naive Lag-1', fontsize=15, pad=12)
    plt.xlabel('Date', fontsize=13)
    plt.ylabel('Wh', fontsize=13)
    plt.grid(alpha=0.4, linestyle=':', linewidth=0.8)
    ax = plt.gca()
...
def save_forecast_plot_variants(plot_df, out_dir, horizon, mase, rmsse, nMAE, nRMSE, r2, smape, pvgis_metrics=None):
    ...
    pvgis_plot_path = os.path.join(out_dir, f'forecast_vs_actual_vs_naive_lag1_vs_PVGIS_{horizon}d.png')
    ...
    try:
        plot_forecast_vs_actual_naive_vs_pvgis(
            ...
            plot_df,
            PVGIS_DAILY_SCALED,
            title=f'Wh Forecast vs Actual vs Naive Lag-1 vs PVGIS ({horizon}d)',
            figsize=(12, 6),
            dpi=150,
        )
    ...
    pvgis_format2_path = os.path.join(out_dir, f'PVGIS_vs_actual_vs_naive_lag1_{horizon}d-format2.png')
    try:
        ...
        plot_forecast_comparison_format2_pvgis(
            ...
            pvgis_series,
            plot_df['NaiveLag1'],
            title=f'PVGIS vs Actual vs Naive Lag-1 ({horizon}d)',
            mase=pvgis_mase,
            rmsse=pvgis_rmsse,
            figsize=(6, 3),
            dpi=300,
...
def generate_forecast_outputs(train_df, test_df, train_value_df, prediction, model, out_dir, horizon):
    ...
    try:
        ...
        print('Failed to save PVGIS vs NaiveLag1 metrics:', e)
...
def main():
    ...
    try:
        ...
        # CLI override for PVGIS-as-future-regressor
        if getattr(args, 'enable_pvgis_fr', None) is not None:
            b = _parse_bool(args.enable_pvgis_fr)
            if b is not None:
                globals()['ENABLE_PVGIS_AS_FUTURE_REGRESSOR'] = b
                applied.append('ENABLE_PVGIS_AS_FUTURE_REGRESSOR')
        ...
        for idx, horizon in enumerate(horizons):
            ...
            if ENABLE_TMY:
                try:
                    ...
                    # Optionally load PVGIS scaled CSV as future_regressor for fit/predict
                    if ENABLE_PVGIS_AS_FUTURE_REGRESSOR:
                        try:
                            pvgis_fit = load_pvgis_future_regressor(PVGIS_DAILY_SCALED, train_df.index)
                            pvgis_pred = load_pvgis_future_regressor(PVGIS_DAILY_SCALED, test_df.index)
                            try:
                                fit_fr_candidate = _merge_fr(fit_fr_candidate, pvgis_fit)
                            except Exception:
                                fit_fr_candidate = pvgis_fit or fit_fr_candidate
                            try:
                                pred_fr_candidate = _merge_fr(pred_fr_candidate, pvgis_pred)
                            except Exception:
                                pred_fr_candidate = pvgis_pred or pred_fr_candidate
                            print('Loaded PVGIS as future regressor (fit/predict):', list(pvgis_fit.columns) if pvgis_fit is not None else [])
                        except Exception as e:
                            print('PVGIS load as future regressor failed, continuing without PVGIS:', e)
            try:
