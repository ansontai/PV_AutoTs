...
# 範例： enable_pvgis_fr = True -> forward to target as --enable_pvgis_fr true
enable_pvgis_fr = False
...
# TARGET = os.path.join(HERE, '6v4b-autoTs_WeatherToDayWh.py')
...
def build_forward_args(parsed):
    ...
    for name in SUPPORTED:
        ...
        if isinstance(val, list):
            # forward lists as comma-separated strings (shell-friendly)
            s = ','.join(map(str, val))
        else:
            s = str(val)
        out.append(f'--{name}')
