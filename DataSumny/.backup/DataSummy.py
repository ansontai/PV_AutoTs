...
def main(argv: Optional[List[str]] = None) -> int:
	...
	# Allow disabling/enabling charts with an environment variable.
	# If PLOTS is set, it overrides the command-line flag.
	env_plots = os.getenv("PLOTS")
	if env_plots is not None:
		plots_enabled = env_plots.strip().lower() not in ("0", "false", "no", "off", "")
		args.charts = plots_enabled
		logger.info("PLOTS environment override -> charts %s", "enabled" if args.charts else "disabled")
