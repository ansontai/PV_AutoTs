...
def compute_doy_regression_scaling_k(train_df: pd.DataFrame, pvgis_df: pd.DataFrame, logger: Logger) -> tuple[pd.Series, pd.DataFrame]:
    """
    計算 DOY 級別的線性迴歸縮放係數。
    對每個 DOY，計算迴歸 obs_Wh = a * pvgis_P + b，計算斜率 a 和截距 b。
    
    Returns:
        (scaling_k_series, coefficients_df)
        - scaling_k_series: 斜率 A（用於構建縮放序列）
        - coefficients_df: 包含 DOY, A_slope, B_intercept, sample_count 的 DataFrame
    """
    logger.info("Computing DOY-based regression scaling factors (A and B)...")
    ...
    # 先按 DOY 彙整成可預期的配對表，避免直接依索引對齊造成空表或欄位遺失。
    combined = pd.DataFrame(
        {
            "obs_Wh": train_tmp.groupby("doy")["obs_Wh"].mean(),
            "pvgis_P": pvgis_tmp.groupby("doy")["pvgis_P"].mean(),
        }
    ).dropna().reset_index()
    
    def fit_regression(group):
        """對每個 DOY 的數據計算迴歸係數（斜率 A 和截距 B）。"""
        n_samples = len(group)
        if n_samples < 2:
            return pd.Series({
                "A_slope": 1.0,
        ...
        try:
            X = group["pvgis_P"].values
            y = group["obs_Wh"].values
            # 使用 polyfit 計算線性迴歸（1 次多項式）
            coeffs = np.polyfit(X, y, 1)
            slope = float(coeffs[0])
            intercept = float(coeffs[1])
            # A 值限制在 [0.1, 10.0]，B 值允許自由範圍
            slope = np.clip(slope, 0.1, 10.0)
            return pd.Series({
                "A_slope": slope,
    ...
    coeff_records: list[dict[str, object]] = []
    if not combined.empty:
        for doy, group in combined.groupby("doy", sort=True):
            result = fit_regression(group)
            coeff_records.append(
                {
                    "doy": int(doy),
                    "A_slope": float(result.get("A_slope", 1.0)),
                    "B_intercept": float(result.get("B_intercept", 0.0)),
                    "sample_count": int(result.get("sample_count", len(group))),
                }
            )

    coeff_df = pd.DataFrame(coeff_records, columns=["doy", "A_slope", "B_intercept", "sample_count"])
    all_doy = pd.DataFrame({"doy": range(1, 367)})
    coeff_df = all_doy.merge(coeff_df, on="doy", how="left")

    coeff_df["A_slope"] = pd.to_numeric(coeff_df["A_slope"], errors="coerce").fillna(1.0).clip(0.1, 10.0)
    coeff_df["B_intercept"] = pd.to_numeric(coeff_df["B_intercept"], errors="coerce").fillna(0.0)
    coeff_df["sample_count"] = pd.to_numeric(coeff_df["sample_count"], errors="coerce").fillna(0).astype(int)

    if coeff_df[["A_slope", "B_intercept", "sample_count"]].isna().any().any():
        raise ValueError("Regression coefficient table still contains NaN values after normalization.")

    valid_group_count = int((coeff_df["sample_count"] >= 2).sum())
    if valid_group_count == 0:
        logger.info("Regression scaling has no DOY groups with >=2 samples; using default slope/intercept values for all DOYs.")
    else:
        logger.info(f"Regression scaling has {valid_group_count}/366 DOY groups with >=2 samples.")

    # 提取斜率作為 Series（用於後續構建縮放序列）
    k = pd.Series(coeff_df["A_slope"].values, index=coeff_df["doy"].values, name="regression_scaling_k")
...
def build_pvgis_p95_scaled_series(
    ...
    pvgis_tmp = pvgis_df[[PVGIS_RAW_P_COLUMN]].copy()
    pvgis_tmp["doy"] = pvgis_tmp.index.dayofyear
    # Use the 95th percentile per DOY
...
def generate_plots_for_forecast_365d_csvs(
    ...
    """
    try:
        forecast_only_mode = actual_series is None or lastvalue_series is None
        pvgis_adj_cols = {
