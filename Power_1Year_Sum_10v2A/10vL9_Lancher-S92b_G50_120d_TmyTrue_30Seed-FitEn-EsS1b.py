...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Launcher for 10vL9 365d forecast')
    ...
    # child_script = script_dir / '10vL9_autots_365d_forecast.py'
    # child_script = script_dir / '10vL9_autots_365d_forecast-EsAll.py'
    child_script = script_dir / '10vL9_autots_365d_forecast-EsS1b.py'
    ...
    # 傳遞是否關閉 TMY 外生變數
    if args.no_tmy_exogenous:
        base_cmd += ['--no_tmy_exogenous']
    # 傳遞 doc-only 模式
    if args.doc_only:
        base_cmd += ['--doc-only']
    # 傳遞 saved-forecast-csv 路徑
    if args.saved_forecast_csv:
        base_cmd += ['--saved-forecast-csv', args.saved_forecast_csv]
