...
#   "mosaic-crosshair",     ### V
#   # "mosaic-mae-0-40",      ### X
#   # "mosaic-weighted-0-40",   ### X
#   "mosaic-weighted-median", ### V
#   # "mosaic-mae-median-0-30", ### X
#   "mosaic-mae-filtered-0-30", ### V
#   "simple",
#   "distance",
#   "subsample"
#   ]
...
def main(args) -> None:
    ...
    try:
        ...
        try:
            ...
            if "out of bounds for int8" in msg or "overflow" in msg:
                logger.info(
                    ...
                    "retrying with fallback ensemble='ENSEMBLE_FALLBACK_SAFE' to avoid mosaic ensemble."
                )
        ...
        # 第二組驗證：為不使用外生變數的預測生成回測結果
        # 對訓練數據的最後 fit_forecast_length 天進行驗證
        logger.info("Running holdout validation for predictions WITHOUT exogenous variables.")
        val_len_no_ex = min(fit_forecast_length, max(14, len(train_df) // 5)) if fit_forecast_length is None else fit_forecast_length
        val_len_no_ex = int(val_len_no_ex)
        
        if len(train_df) > val_len_no_ex + 30:
            train_part_no_ex = train_df.iloc[:-val_len_no_ex].copy()
            val_part_no_ex = train_df.iloc[-val_len_no_ex:].copy()
            
            try:
                val_fcst_no_ex = model.predict(forecast_length=val_len_no_ex, future_regressor=None)
                if TARGET_COLUMN not in val_fcst_no_ex.forecast.columns and len(val_fcst_no_ex.forecast.columns) > 0:
                    val_fcst_no_ex.forecast = val_fcst_no_ex.forecast.rename(columns={val_fcst_no_ex.forecast.columns[0]: TARGET_COLUMN})
                
                val_fcst_values_no_ex = val_fcst_no_ex.forecast[TARGET_COLUMN].values if TARGET_COLUMN in val_fcst_no_ex.forecast.columns else val_fcst_no_ex.forecast.iloc[:, 0].values
                val_fcst_aligned_no_ex = pd.Series(
                    val_fcst_values_no_ex[:len(val_part_no_ex)],
                    index=val_part_no_ex.index,
                    name=TARGET_COLUMN,
                )
                
                holdout_metrics_no_ex = compute_holdout_metrics(
                    actual=val_part_no_ex[TARGET_COLUMN],
                    pred=val_fcst_aligned_no_ex,
                    train_series=train_part_no_ex[TARGET_COLUMN],
                )
                holdout_metrics_no_ex["holdout_length"] = float(val_len_no_ex)
                
                holdout_result_no_ex = HoldoutBacktestResult(
                    metrics=holdout_metrics_no_ex,
                    holdout_length=val_len_no_ex,
                    actual=val_part_no_ex[TARGET_COLUMN].copy(),
                    forecast=val_fcst_aligned_no_ex.copy(),
                    lastvalue=pd.Series(
                        np.repeat(float(train_part_no_ex[TARGET_COLUMN].iloc[-1]), len(val_part_no_ex)),
                        index=val_part_no_ex.index,
                        name=TARGET_COLUMN,
                    ),
                )
                logger.info(f"Holdout validation (without exogenous) completed for {val_len_no_ex} days")
            except Exception as exc:
                logger.info(f"Failed to generate holdout validation for predictions without exogenous: {exc}")
                holdout_result_no_ex = None
                holdout_metrics_no_ex = {}
        else:
            logger.info(f"Not enough training data ({len(train_df)} rows) for holdout validation without exogenous; skipping.")
            holdout_result_no_ex = None
            holdout_metrics_no_ex = {}
