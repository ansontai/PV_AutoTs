...
def plot_forecast_comparison(plot_path, index, y_true, y_pred, y_naive, title=None, figsize=(12, 6), dpi=150,
    ...
    plt.plot(index, y_naive, label=baseline_label, linewidth=2, linestyle='--')
...
def plot_forecast_comparison_legacy(plot_path, index, y_true, y_pred, y_naive,
                                    ...
                                    baseline_label='Naive Lag-1',
                                    forecast_label='AutoTS Forecast'):
    import matplotlib
    matplotlib.use('Agg')
    import matplotlib.pyplot as plt
    ...
    plt.plot(index, y_pred, label=forecast_label, color='dimgray', linewidth=2.5)
    plt.plot(index, y_naive, label=baseline_label, color='gray', linewidth=2, linestyle='--')
...
def compute_forecast_scores(y_true, y_pred, train_series, baseline_lastvalue=None):
    ...
    mase_lastvalue = np.nan
    if baseline_lastvalue is not None:
        try:
            baseline_vals = np.asarray(baseline_lastvalue, dtype=float)
            if baseline_vals.shape == y_true.shape and baseline_vals.size > 0:
                mae_lastvalue = float(mean_absolute_error(y_true, baseline_vals))
                mase_lastvalue = float(mae / mae_lastvalue) if mae_lastvalue != 0 else np.nan
        except Exception:
            mase_lastvalue = np.nan
    ...
    return {
        ...
        'MASE_lastValue': float(mase_lastvalue) if not np.isnan(mase_lastvalue) else None,
...
def VsPvgis_LoadMonthDayMap(pvgis_path, value_col='P_mapped_Wh', date_col='date'):
    if not pvgis_path:
        return None
    try:
        if not os.path.exists(pvgis_path):
            print('PVGIS file not found:', pvgis_path)
            return None
    except Exception:
        return None

    try:
        df = pd.read_csv(pvgis_path)
    except Exception as e:
        print('Failed to read PVGIS CSV:', e)
        return None
    ...
    if date_col is None or date_col not in df.columns:
        print('PVGIS date column not found in CSV')
        return None

    if value_col not in df.columns:
        print(f'PVGIS value column not found: {value_col}')
        return None
    ...
    try:
        ...
        if df.empty:
            return None
        ser = df.set_index(date_col)[value_col].sort_index()
        grouped = ser.groupby([ser.index.month, ser.index.day]).mean()
        if grouped.empty:
            return None
        return grouped
    except Exception as e:
        print('Failed to parse PVGIS data:', e)
        return None


def VsPvgis_AlignByMonthDay(month_day_map, target_index):
    if month_day_map is None:
        return None
    try:
        idx = pd.Index(target_index)
        values = []
        for m, d in zip(idx.month, idx.day):
            try:
                values.append(float(month_day_map.loc[(m, d)]))
            except Exception:
...
def VsPvgis_FilterValidPairs(y_true, y_pred, y_lastvalue=None):
    try:
    ...
    if y_lastvalue is not None:
        try:
            y_last_arr = np.asarray(y_lastvalue, dtype=float)
            mask = mask & np.isfinite(y_last_arr)
        except Exception:
            y_last_arr = None
    else:
        y_last_arr = None
    ...
    if y_last_arr is not None:
        return y_true_arr[mask], y_pred_arr[mask], y_last_arr[mask]
    return y_true_arr[mask], y_pred_arr[mask], None


def VsPvgis_ComputeScores(y_true, y_pred, train_series, y_lastvalue=None):
    y_true_valid, y_pred_valid, y_last_valid = VsPvgis_FilterValidPairs(y_true, y_pred, y_lastvalue)
    if y_true_valid is None or y_pred_valid is None:
        return None
    return compute_forecast_scores(
        y_true_valid,
        y_pred_valid,
        train_series,
        baseline_lastvalue=y_last_valid,
    )


def VsPvgis_PlotComparison(plot_path, index, y_true, y_pvgis, y_lastvalue,
                           mase=None, rmsse=None, smape=None, title=None):
...
def save_iteration_best_artifacts(output_dir, timestamp, iteration, horizon_results):
    ...
    mase_lag1_candidates = []
    for r in valid_results:
        v = r.get('mase_lag1')
        if isinstance(v, (int, float)):
            mase_lag1_candidates.append(r)
    mase_lag1_best = min(mase_lag1_candidates, key=lambda x: float(x.get('mase_lag1'))) if mase_lag1_candidates else None

    mase_lastvalue_candidates = []
    for r in valid_results:
        v = r.get('mase_lastvalue')
        if isinstance(v, (int, float)):
            mase_lastvalue_candidates.append(r)
    mase_lastvalue_best = min(mase_lastvalue_candidates, key=lambda x: float(x.get('mase_lastvalue'))) if mase_lastvalue_candidates else None

    best_for_artifacts = mase_lastvalue_best or mase_lag1_best

    summary = {
        'timestamp': timestamp,
        'iteration': int(iteration),
        'mase_lastvalue_best': {
            'horizon': mase_lastvalue_best.get('horizon') if mase_lastvalue_best else None,
            'mase_lastvalue': mase_lastvalue_best.get('mase_lastvalue') if mase_lastvalue_best else None,
            'best_model_name': mase_lastvalue_best.get('best_model_name') if mase_lastvalue_best else None,
            'artifacts': mase_lastvalue_best.get('artifacts') if mase_lastvalue_best else None,
            'last_value_index': mase_lastvalue_best.get('last_value_index') if mase_lastvalue_best else None,
            'last_value_value': mase_lastvalue_best.get('last_value_value') if mase_lastvalue_best else None,
        },
        'mase_lag1_best': {
            'horizon': mase_lag1_best.get('horizon') if mase_lag1_best else None,
            'mase_lag1': mase_lag1_best.get('mase_lag1') if mase_lag1_best else None,
            'best_model_name': mase_lag1_best.get('best_model_name') if mase_lag1_best else None,
            'artifacts': mase_lag1_best.get('artifacts') if mase_lag1_best else None,
            'last_value_index': mase_lag1_best.get('last_value_index') if mase_lag1_best else None,
            'last_value_value': mase_lag1_best.get('last_value_value') if mase_lag1_best else None,
        },
        'autots_best_by_horizon': [
            {
                ...
                'mase_lastvalue': r.get('mase_lastvalue'),
                'artifacts': r.get('artifacts'),
                'last_value_index': r.get('last_value_index'),
                'last_value_value': r.get('last_value_value'),
            }
            for r in valid_results
        ],
    }
    ...
    if best_for_artifacts:
        try:
            src_csv = ((best_for_artifacts.get('artifacts') or {}).get('best_template_csv_path'))
            src_json = ((best_for_artifacts.get('artifacts') or {}).get('best_template_json_path'))
...
def run_horizon(AutoTS, ser, horizon, output_dir, timestamp, count, n_jobs, max_generations,
                ...
                enable_pvgis=True, pvgis_month_day_map=None):
    ...
    def _run():
        ...
        try:
            ...
            if len(y_pred) == len(y_true):
                last_value = float(train_ser.iloc[-1, 0])
                # extract the index/date corresponding to the last training value
                last_value_index = None
                try:
                    idx = train_ser.index[-1]
                    try:
                        last_value_index = idx.isoformat()
                    except Exception:
                        last_value_index = str(idx)
                except Exception:
                    last_value_index = None

                y_naive = np.concatenate(([last_value], y_true[:-1])) if horizon > 1 else np.array([last_value])
                y_lastvalue = np.full_like(y_true, fill_value=last_value, dtype=float)
                scores = compute_forecast_scores(
                    y_true,
                    y_pred,
                    train_ser.iloc[:, 0].astype(float).values,
                    baseline_lastvalue=y_lastvalue,
                )
                # append last value metadata to scores for later auditing
                try:
                    scores['last_value_index'] = last_value_index
                except Exception:
                    pass
                try:
                    scores['last_value_value'] = float(last_value)
                except Exception:
                    pass
                ...
                try:
                    horizon_result['mase_lastvalue'] = scores.get('MASE_lastValue')
                except Exception:
                    horizon_result['mase_lastvalue'] = None
                try:
                    horizon_result['last_value_index'] = scores.get('last_value_index')
                except Exception:
                    horizon_result['last_value_index'] = None
                try:
                    horizon_result['last_value_value'] = scores.get('last_value_value')
                except Exception:
                    horizon_result['last_value_value'] = None
                ...
                try:
                    plot_path_lv = os.path.join(horizon_dir, f'forecast_vs_actual_vs_lastvalue_{horizon}.png')
                    plot_forecast_comparison(plot_path_lv, test_ser.index, y_true, y_pred, y_lastvalue,
                                             title=f'Wh Forecast vs Actual vs LastValue ({horizon}d)',
                                             baseline_label='LastValue')
                    print('Saved LastValue comparison chart to', plot_path_lv)
                except Exception as e:
                    print('Failed to save LastValue comparison chart:', e)

                try:
                    plot_path_lv2 = os.path.join(horizon_dir, f'AutoTS_forecast_vs_actual_vs_lastvalue_{horizon}-format2.png')
                    plot_forecast_comparison_legacy(plot_path_lv2, test_ser.index, y_true, y_pred, y_lastvalue,
                                                    mase=scores.get('MASE_lastValue'),
                                                    rmsse=scores.get('RMSSE'),
                                                    smape=scores.get('SMAPE(%)'),
                                                    title=f'AutoTS Forecast vs Actual vs LastValue ({horizon}d) - format2',
                                                    baseline_label='LastValue')
                    print('Saved AutoTS comparison chart (format2) to', plot_path_lv2)
                except Exception as e:
                    print('Failed to save AutoTS comparison chart format2:', e)

                try:
                    if enable_pvgis:
                        pvgis_values = VsPvgis_AlignByMonthDay(pvgis_month_day_map, test_ser.index)
                        if pvgis_values is None:
                            raise ValueError('PVGIS alignment failed')
                        pvgis_scores = VsPvgis_ComputeScores(
                            y_true,
                            pvgis_values,
                            train_ser.iloc[:, 0].astype(float).values,
                            y_lastvalue,
                        )
                        plot_path_pvgis = os.path.join(
                            horizon_dir,
                            f'PVGIS_forecast_vs_actual_vs_lastvalue_{horizon}-format2.png'
                        )
                        VsPvgis_PlotComparison(
                            plot_path_pvgis,
                            test_ser.index,
                            y_true,
                            pvgis_values,
                            y_lastvalue,
                            mase=pvgis_scores.get('MASE_lastValue') if pvgis_scores else None,
                            rmsse=pvgis_scores.get('RMSSE') if pvgis_scores else None,
                            smape=pvgis_scores.get('SMAPE(%)') if pvgis_scores else None,
                            title=f'PVGIS Forecast vs Actual vs LastValue ({horizon}d) - format2',
                        )
                        print('Saved PVGIS comparison chart (format2) to', plot_path_pvgis)
                except Exception as e:
                    print('Failed to save PVGIS comparison chart format2:', e)
            else:
...
def main():
    ...
    parser.add_argument('--enable_pvgis', type=str, default='True', help='Enable PVGIS comparison chart (true/false)')
    parser.add_argument('--pvgis_file', type=str,
                        ...
                        help='PVGIS daily scaled CSV path (uses P_mapped_Wh)')
    ...
    parsed_pvgis = _parse_bool(args.enable_pvgis)
    if parsed_pvgis is None:
        parser.error("--enable_pvgis must be true or false")
    args.enable_pvgis = parsed_pvgis
    ...
    pvgis_month_day_map = None
    if args.enable_pvgis:
        pvgis_month_day_map = VsPvgis_LoadMonthDayMap(args.pvgis_file)
        if pvgis_month_day_map is None:
            print('PVGIS month/day mapping unavailable; PVGIS chart will be skipped.')
        ...
        try:
            for horizon in horizons:
                horizon_result = run_horizon(AutoTS, ser, horizon, output_dir, timestamp, count,
                                             ...
                                             enable_pvgis=args.enable_pvgis,
                                             pvgis_month_day_map=pvgis_month_day_map)
