...
DEFAULT_SEEDS_FILE_NAME = 'autots_seeds_20260420_034832-2seed.csv'
...
def collect_seed_mase_summary(output_root: Path, mode_tags: list[str]):
    """Collect one summary row per seed with both exogenous and non-exogenous MASE values."""
    latest_by_seed_mode = {}
    try:
        candidates = list(Path(output_root).rglob('effective_settings_*.json'))
    except Exception:
        return []
    ...
    for settings_path in candidates:
        ...
        mode_tag = str(settings.get('output_mode_tag') or settings_path.parent.name or '').strip()
        if mode_tag not in mode_tags:
            continue

        try:
            seed_value = int(settings.get('random_seed'))
        except (TypeError, ValueError):
            continue

        mtime = settings_path.stat().st_mtime if settings_path.exists() else 0
        previous = latest_by_seed_mode.get((seed_value, mode_tag))
        if previous is None or mtime >= previous[0]:
            latest_by_seed_mode[(seed_value, mode_tag)] = (mtime, settings)

    if not latest_by_seed_mode:
        return []

    seed_values = sorted({seed for seed, _mode in latest_by_seed_mode.keys()})
    rows = []
    for seed_value in seed_values:
        with_ex_settings = latest_by_seed_mode.get((seed_value, 'with_exogenous'))
        without_ex_settings = latest_by_seed_mode.get((seed_value, 'without_exogenous'))

        with_ex_value = ''
        without_ex_value = ''
        if with_ex_settings is not None:
            with_ex_payload = with_ex_settings[1]
            with_ex_value = _csv_cell(
                with_ex_payload.get('holdout_mase_with_exogenous', with_ex_payload.get('holdout_mase', ''))
            )
        if without_ex_settings is not None:
            without_ex_payload = without_ex_settings[1]
            without_ex_value = _csv_cell(
                without_ex_payload.get('holdout_mase_without_exogenous', without_ex_payload.get('holdout_mase', ''))
            )

        rows.append({
            'random_seed': seed_value,
            'holdout_mase_with_exogenous': with_ex_value,
            'holdout_mase_without_exogenous': without_ex_value,
        })
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Launcher for 10vL10c 365d forecast')
    ...
    child_script = script_dir / '10vL10c_autots_365d_forecast.py'
...
# After running modes, attempt to summarize per-seed MASE values into one CSV
try:
    summary_rows = collect_seed_mase_summary(child_output_root, [mode_tag for mode_tag, _ in mode_specs])

    if summary_rows:
        summary_ts = datetime.now().strftime("%Y%m%d_%H%M%S")
        summary_file = child_output_root / f"seed_mase_summary_{summary_ts}.csv"
        with open(summary_file, 'w', newline='', encoding='utf-8-sig') as f:
            writer = csv.DictWriter(f, fieldnames=list(summary_rows[0].keys()))
            writer.writeheader()
            for r in summary_rows:
                writer.writerow(r)
        print(f'Launcher: wrote seed MASE summary CSV: {summary_file}')
    else:
