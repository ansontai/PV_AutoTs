...
# TRAIN_CSV_PATH = BASE_DIR / "input" / "SolarRecord(260228)_d_forWh_WithCodis.csv"
...
FUTURE_REG_CSV_PATH = BASE_DIR / "input" / "tmy_24.148_120.703_2005_2023[UTC+8][daily][mapped].csv"
...
DEFAULT_MAX_GENERATIONS = 2  # 可調整以平衡速度與預測品質
...
#     #X 'SeasonalNaive', 
# ]
DEFAULT_MODEL_LIST = 'default' # 預設模型組合（AutoTS 內建的預設組合）,

DEFAULT_NUM_VALIDATIONS = 3 # 交叉驗證次數（增加可提升預測穩定性，但會顯著增加運行時間）
...
def _read_future_regressor(future_reg_csv_path: Path) -> pd.DataFrame:
    ...
    # Handle case-insensitive date column name
    date_col = None
    ...
    if date_col is None:
        raise ValueError("Future regressor CSV must contain Date (or date) column")

    # Standardize to "Date" column name
    if date_col != "Date":
        reg = reg.rename(columns={date_col: "Date"})
