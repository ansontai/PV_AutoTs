...
TRAIN_CSV_PATH = BASE_DIR / "input" / "SolarRecord(260228)_d_forWh_WithCodis[date].csv"
# FUTURE_REG_CSV_PATH = BASE_DIR / "input" / "forecast_weather_1y.csv"
FUTURE_REG_CSV_PATH = BASE_DIR / "input" / "tmy_24.148_120.703_2005_2023[UTC+8][daily][mapped][dateAdj].csv"
...
def _read_train_target_and_reg(train_csv_path: Path) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if date_col is None or "Wh" not in df.columns:
        raise ValueError("Training CSV must contain date column ('Date', 'date' or 'LocalTime') and 'Wh' column")
    ...
    regressor_exclude = {"Date", "date", "LocalTime", "Wh"}
...
def _read_future_regressor(future_reg_csv_path: Path) -> pd.DataFrame:
    ...
    date_col = (
        ...
        else ("date" if "date" in reg.columns else ("LocalTime" if "LocalTime" in reg.columns else None))
    )
    if date_col is None:
        raise ValueError("Future regressor CSV must contain date column ('Date', 'date' or 'LocalTime')")

    reg["Date"] = pd.to_datetime(reg[date_col], errors="coerce")
    reg = reg.dropna(subset=["Date"]).sort_values("Date").drop_duplicates(subset=["Date"], keep="last")

    reg_cols = [c for c in reg.columns if c not in {"Date", "date", "LocalTime"}]
    for col in reg_cols:
        reg[col] = pd.to_numeric(reg[col], errors="coerce")
...
def main() -> None:
    ...
    train_reg = train_reg.apply(pd.to_numeric, errors="coerce").astype("float64")
    future_reg = future_reg.apply(pd.to_numeric, errors="coerce").astype("float64")

    all_nan_cols = train_reg.columns[train_reg.isna().all() | future_reg.isna().all()]
    if len(all_nan_cols) > 0:
        print(f"Dropping all-NaN regressor columns: {list(all_nan_cols)}")
        train_reg = train_reg.drop(columns=all_nan_cols)
        future_reg = future_reg.drop(columns=all_nan_cols)

    if train_reg.shape[1] == 0:
        raise ValueError("No usable numeric regressor columns after coercion/drop")
    ...
    for i in range(0, horizon, ROLLING_FORECAST_LENGTH):
        ...
        chunk_future_reg = chunk_future_reg.apply(pd.to_numeric, errors="coerce").astype("float64")
        chunk_future_reg = chunk_future_reg.reindex(columns=history_reg.columns)
        ...
        non_num_cols = [c for c in history_reg.columns if not pd.api.types.is_numeric_dtype(history_reg[c])]
        if non_num_cols:
            print(f"WARNING non-numeric columns before fit: {non_num_cols}")
        print(f"history_reg shape={history_reg.shape}, chunk_future_reg shape={chunk_future_reg.shape}")
