...
produces three scaled columns for `P`:
	- P_Wh_min_max_scaled  : P mapped to Wh range (map P_min->Wh_min and P_max->Wh_max)
	- P_max_normalization  : P normalized to [0,1] by P_max (PVGIS hourly peak)
	- P_wh_max_scaled      : (P / P_max) * Wh_max (range-mapped to SolarRecord ceiling)
...
def main():
	...
	p.add_argument(
		"--pvgis",
		default=(
			"PVGIS/raw/Timeseries_24.148_120.703_E5_0kWp_crystSi_25_35deg_1deg_2005_2005[UTC+8][daily][scaled][vB].csv"
		),
	)
	...
	pvgis_hour["P_Wh_min_max_scaled"] = (
		(pvgis_hour["P"] - P_min) / (P_max - P_min) * (wh_max - wh_min) + wh_min
	)
	pvgis_hour["P_max_normalization"] = pvgis_hour["P"] / P_max
	pvgis_hour["P_wh_max_scaled"] = (pvgis_hour["P"] / P_max) * wh_max
