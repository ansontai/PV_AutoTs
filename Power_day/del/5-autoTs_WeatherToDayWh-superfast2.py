...
def main():
    ...
    for horizon in horizons:
        ...
        for k, v in scores.items():
            print(f'{k}: {v}')
        try:
            plot_path = os.path.join(out_dir, f'forecast_vs_actual_vs_naive_mean_{horizon}d.png')
            ...
            plt.title(f'Wh Forecast vs Actual vs Naive Mean ({horizon}d)')
            plt.xlabel('Date')
            plt.ylabel('Wh')
            plt.grid(alpha=0.3)
            plt.legend()
        ...
        # 儲存訓練完成的最佳模型 template
        try:
            best_model_path = os.path.join(out_dir, 'best_models.csv')
            ...
            print('Best model template saved to', best_model_path)
        except Exception as e:
