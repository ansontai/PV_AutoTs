...
# Optional exogenous regressors CSV (日頻)
DEFAULT_EXOG = os.path.normpath(os.path.join(os.path.dirname(__file__), '..', 'csv', '2000--202602-d-forWh_4b.csv'))
...
def main():
    ...
    # === 讀取外生變數（exogenous regressors）CSV（如果存在） ===
    exog_df = None
    try:
        if os.path.exists(DEFAULT_EXOG):
            tmp = pd.read_csv(DEFAULT_EXOG, low_memory=True)
            # detect date column
            date_col = None
            if 'Date' in tmp.columns:
                date_col = 'Date'
            elif 'LocalTime' in tmp.columns:
                date_col = 'LocalTime'
            else:
                # fallback: first column if it looks like a date
                date_col = tmp.columns[0]
            tmp[date_col] = pd.to_datetime(tmp[date_col], errors='coerce')
            tmp = tmp.dropna(subset=[date_col]).sort_values(date_col).drop_duplicates(subset=[date_col], keep='last')
            tmp = tmp.set_index(date_col).asfreq('D')
            ...
            try:
                ...
                pass
            exog_df = tmp
            print('Loaded exogenous regressors from', DEFAULT_EXOG, 'columns:', list(exog_df.columns))
    except Exception as e:
        print('Failed to load exogenous regressors:', e)
    ...
    while True:
        ...
        for idx, horizon in enumerate(horizons):
            ...
            # align regressor slices for this horizon if exog available
            train_reg = None
            future_reg = None
            if exog_df is not None:
                try:
                    # reindex exog to train/test index and keep numeric columns only
                    common_cols = [c for c in exog_df.columns if c not in ('LocalTime',)]
                    if len(common_cols) > 0:
                        train_reg = exog_df[common_cols].reindex(train_df.index).ffill().bfill()
                        future_reg = exog_df[common_cols].reindex(test_df.index).ffill().bfill()
                        # if all NaN, drop to None
            ...
            # AutoTS.fit may accept `future_regressor` depending on version
            try:
                if train_reg is not None:
                    model = model.fit(train_wide, future_regressor=train_reg)
                else:
                    model = model.fit(train_wide)
            except TypeError:
                # fallback if this AutoTS version doesn't accept future_regressor in fit
                model = model.fit(train_wide)

            print('Generating prediction...')
            try:
                # If model was trained with regressors but future_reg is missing,
                # attempt to construct a sensible future_reg from train_reg.
                if train_reg is not None and future_reg is None:
                    try:
                        cand = train_reg.reindex(test_df.index).ffill().bfill()
                        if cand.isna().all(axis=None):
                            # fallback: repeat last known regressor row
                            last_row = train_reg.dropna().tail(1)
                            if not last_row.empty:
                                cand = pd.DataFrame([last_row.iloc[0].to_dict()])
                                cand = pd.concat([cand] * len(test_df), ignore_index=True)
                                cand.index = test_df.index
                                print('Warning: future_reg was missing; filled with last available regressor values.')
                            else:
                                cand = None
                        else:
                            cand.index = test_df.index
                        ...
                        print('Warning: failed to construct future_reg from train_reg:', e)

                if future_reg is not None:
                    prediction = model.predict(forecast_length=horizon, future_regressor=future_reg)
                else:
                    prediction = model.predict(forecast_length=horizon)
                forecast = getattr(prediction, 'forecast', prediction)
            except Exception as e:
                # Common failure: model was trained with future_regressor but None was passed.
                msg = str(e)
                if ('NoneType' in msg and 'reindex' in msg) or 'future_regressor not provided' in msg:
                    # Try to build a fallback future_reg from exog_df or train_reg.
                    fallback = None
                    try:
                        if train_reg is not None:
                            last_row = train_reg.dropna().tail(1)
                            if not last_row.empty:
                                fallback = pd.DataFrame([last_row.iloc[0].to_dict()])
                                fallback = pd.concat([fallback] * len(test_df), ignore_index=True)
                                fallback.index = test_df.index
                        if fallback is None and exog_df is not None:
                            common_cols = [c for c in exog_df.columns if c not in ('LocalTime',)]
                            if common_cols:
                                fallback = exog_df[common_cols].reindex(test_df.index).ffill().bfill()
                                if fallback.isna().all(axis=None):
                                    fallback = None
                    except Exception:
                        fallback = None

                    # If we still don't have a fallback, try to build one from exog_df
                    if fallback is None and exog_df is not None:
                        try:
                            common_cols = [c for c in exog_df.columns if c not in ('LocalTime',)]
                            if common_cols:
                                # try last non-NaN row up to end of training
                                last_vals = None
                                try:
                                    last_vals = exog_df[common_cols].loc[:train_df.index.max()].ffill().dropna(how='all').tail(1)
                                except Exception:
                                    last_vals = None
                                if last_vals is not None and not last_vals.empty:
                                    fallback = pd.DataFrame([last_vals.iloc[0].to_dict()] * len(test_df), index=test_df.index)
                                else:
                                    # final resort: use last available per-column value or 0
                                    vals = {}
                                    for c in common_cols:
                                        col = exog_df[c].dropna()
                                        if len(col) > 0:
                                            vals[c] = float(col.iloc[-1])
                                        else:
                                            vals[c] = 0.0
                                    fallback = pd.DataFrame([vals] * len(test_df), index=test_df.index)
                        except Exception:
                            fallback = None

                    if fallback is not None:
                        try:
                            print('Retrying predict with constructed fallback future_regressor')
                            prediction = model.predict(forecast_length=horizon, future_regressor=fallback)
                            forecast = getattr(prediction, 'forecast', prediction)
                        except Exception as e2:
                            print('Retry with fallback future_regressor failed:', e2)
                            raise
                    else:
                        # give up and re-raise original exception for visibility
                        raise
                else:
                    raise
            try:
