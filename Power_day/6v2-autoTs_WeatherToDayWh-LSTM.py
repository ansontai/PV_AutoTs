...
# Optional future regressor CSV paths (set to None or a filename relative to script's csv/ folder)
# Example: '2000--202602-d-forWh_4b.csv' for training, 'forecast_weather_1y.csv' for prediction
FUTURE_REGRESSOR_TRAIN = '2000--202602-d-forWh_4b.csv'
FUTURE_REGRESSOR_PREDICT = 'forecast_weather_1y.csv'
    ...
    while True:
        ...
        for idx, horizon in enumerate(horizons):
            ...
            ats_kwargs = dict(
                ...
                frequency='D',
                ensemble='simple', # 以 simple ensemble 為主，避免被單一模型（例如 Theta）主導預測結果，導致預測結果被抹平成水平線；當然，如果你的資料真的很適合平線模型，simple ensemble 也會選擇平線模型，但它不會讓平線模型「獨大」，因此能夠在很多情況下避免預測結果被抹平成水平線。
                # model_list=[
                #   # 'ETS', # 容易變成水平線的模型
                #   'Theta', # 「幾乎不會是水平線」的安全模型
                #   'ARIMA',
            ...
            model = AutoTS(model_list=model_list_arg, **safe_ats_kwargs)
            ...
            try:
                ...
                # Retry with a conservative fallback: fewer generations, single validation
                try:
                    print('Retrying AutoTS.fit with conservative settings (num_validations=1, max_generations=1, n_jobs=1)')
                    fallback = AutoTS(model_list='default', n_jobs=1, max_generations=1, num_validations=1)
                    fallback.forecast_length = horizon
                    if fit_fr is not None:
                        fallback = fallback.fit(train_wide, future_regressor=fit_fr)
                    else:
                        fallback = fallback.fit(train_wide)
                    model = fallback
                    print('Fallback fit succeeded')
                except Exception as e2:
                    print('Fallback AutoTS.fit also failed, skipping this horizon:', e2)
                    print(traceback.format_exc())
                    continue
