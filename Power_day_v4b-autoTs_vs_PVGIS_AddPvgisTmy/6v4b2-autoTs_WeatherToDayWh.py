...
FUTURE_REGRESSOR_PREDICT = os.path.join(INPUT_DIR, 'tmy_24.148_120.703_2005_2023[UTC+8][daily][mapped][dateAdj].csv')
# TMY_PATH = os.path.join(INPUT_DIR, 'tmy_24.148_120.703_2005_2023[UTC+8][daily][mapped][dateAdj].csv')
...
# --- TMY (典型氣象年) external regressor support ---
# Set to True to load `input/tmy_24.148_120.703_2005_2023[UTC+8][daily].csv`
# and use its numeric columns (or those listed in `TMY_USE_COLUMNS`) as
# future_regressor for fit/predict to potentially improve AutoTS accuracy.
ENABLE_TMY = True
# TMY_FILENAME = 'tmy_24.148_120.703_2005_2023[UTC+8][daily].csv'
TMY_FILENAME = 'tmy_24.148_120.703_2005_2023[UTC+8][daily][mapped][dateAdj].csv'
TMY_PATH = os.path.join(INPUT_DIR, TMY_FILENAME)
# If None -> auto-select numeric columns; otherwise set to list of column names
TMY_USE_COLUMNS = None

def plot_forecast_comparison(plot_path, index, y_true, y_pred, y_naive, title=None, figsize=(12, 6), dpi=150):
    """Save a simple Forecast vs Actual vs LastValueNaive line chart.
    ...
    plt.plot(index, y_naive, label='LastValueNaive', linewidth=2, linestyle='--')
    plt.title(title or 'Wh Forecast vs Actual vs LastValueNaive')
    plt.xlabel('Date')
    plt.ylabel('Wh')
    plt.grid(alpha=0.3)
    plt.legend()
...
def resolve_pvgis_daily_scaled_path():
    ...
    if os.path.exists(preferred):
        return preferred
    candidates = sorted(glob.glob(os.path.join(INPUT_DIR, 'Timeseries_*scaled*.csv')))
    if candidates:
        print('Resolved PVGIS scaled CSV by glob:', candidates[0])
        return candidates[0]
    return preferred

PVGIS_DAILY_SCALED = resolve_pvgis_daily_scaled_path()
...
def plot_forecast_comparison_legacy(plot_path, index, y_true, y_pred, y_naive,
    ...
    """Save a legacy gray-styled Forecast vs Actual vs LastValueNaive chart.
    ...
    plt.plot(index, y_naive, label='LastValueNaive', color='gray', linewidth=2, linestyle='--')
    plt.title(title or 'Wh Forecast vs Actual vs LastValueNaive', fontsize=15, pad=12)
...
def plot_forecast_comparison_format3(plot_path, index, y_true, y_pred, y_naive,
    ...
    plt.plot(index, y_pred, label='AutoTS Forecast', color='dimgray', linewidth=2.5)
    plt.plot(index, y_naive, label='LastValueNaive', color='gray', linewidth=2, linestyle='--')
    plt.title(title or 'Wh Forecast vs Actual vs LastValueNaive', fontsize=12)
    plt.xlabel('Date')
    plt.ylabel('Wh')
...
def compute_forecast_scores(y_true, y_pred, train_series, baseline_lastvalue=None):
    ...
    Returns:
    - scores: dict with MAE, MASE_lag1, MASE_lastValue, RMSSE, SMAPE(%), R2
    - mae, mase, rmsse, smape: numeric values for convenience (may be NaN)
    """
    import numpy as _np
    ...
    mase_lastvalue = _np.nan
    if baseline_lastvalue is not None:
        try:
            baseline_vals = _np.asarray(baseline_lastvalue, dtype=float)
            if baseline_vals.shape == y_true.shape and baseline_vals.size > 0:
                mae_lastvalue = float(_mae(y_true, baseline_vals))
                mase_lastvalue = float(mae / mae_lastvalue) if mae_lastvalue != 0 else _np.nan
        except Exception:
            mase_lastvalue = _np.nan
    ...
    scores = {
        ...
        'MASE_lastValue': float(mase_lastvalue) if not _np.isnan(mase_lastvalue) else None,
...
def save_pvgis_vs_naive_metrics(train_df, test_df, out_dir, horizon, pvgis_csv_path=PVGIS_DAILY_SCALED):
    """Compute PVGIS and naive-lag1 metrics and save them as JSON."""
    y_true, y_naive = compute_naive_baseline(train_df, test_df)
    y_lastvalue = compute_lastvalue_baseline(train_df, test_df)
    pvgis_series = load_aligned_pvgis_series(test_df.index, pvgis_csv_path)
    ...
    pvgis_scores, _, _, _, _ = compute_forecast_scores(
        ...
        baseline_lastvalue=y_lastvalue,
    )
    ...
    lastvalue_scores, _, _, _, _ = compute_forecast_scores(
        y_true.astype(float),
        y_lastvalue.astype(float),
        train_df['Wh'].astype(float).values,
        baseline_lastvalue=y_lastvalue,
    )
    ...
    pvgis_metrics = {
        ...
        'LastValueNaive': lastvalue_scores,
    }

    pvgis_metrics_path = os.path.join(out_dir, f'PVGIS_vs_lastValueNaive_metrics_{horizon}d.json')
    with open(pvgis_metrics_path, 'w', encoding='utf-8') as f:
        json.dump(pvgis_metrics, f, ensure_ascii=False, indent=2)
    print('Saved PVGIS vs LastValueNaive metrics to', pvgis_metrics_path)
    return pvgis_metrics


def compute_lastvalue_baseline(train_df, test_df):
    """Compute last-value baseline for forecast evaluation."""
    train_wh = train_df['Wh'].astype(float).values
    y_true = test_df['Wh'].astype(float).values
    if train_wh.size == 0:
        return np.full_like(y_true, np.nan, dtype=float)
    return np.full_like(y_true, fill_value=train_wh[-1], dtype=float)


def load_aligned_pvgis_series(index, pvgis_csv_path):
    """Load PVGIS P_mapped_Wh and align it to the given datetime index."""
    if not os.path.exists(pvgis_csv_path):
...
def plot_forecast_vs_actual_naive_vs_pvgis(
        plot_path, plot_df, pvgis_csv_path,
        title=None, figsize=(12, 6), dpi=150):
    """Save a chart comparing Actual / Forecast / LastValueNaive vs PVGIS."""
    print('DEBUG: plot_forecast_vs_actual_naive_vs_pvgis called with', pvgis_csv_path)
    try:
        pvgis_series = load_aligned_pvgis_series(plot_df.index, pvgis_csv_path)
    except Exception as e:
        print('Failed to load PVGIS series:', e)
        return
    ...
    plt.plot(plot_df.index, plot_df['LastValueNaive'], label='LastValueNaive', linewidth=2, linestyle='--')
    plt.plot(plot_df.index, pvgis_series, label='PVGIS P_mapped_Wh', linewidth=2, linestyle=':')
    plt.title(title or 'Wh Forecast vs Actual vs LastValueNaive vs PVGIS')
    plt.xlabel('Date')
    plt.ylabel('Wh')
    plt.grid(alpha=0.3)
    ax = plt.gca()
    ...
    plt.savefig(plot_path, dpi=dpi)
    plt.close()
    print('DEBUG: saved PVGIS comparison chart to', plot_path)


def plot_forecast_comparison_format2_pvgis(plot_path, index, y_true, y_pvgis, y_naive,
                                           title=None, mase=None, rmsse=None,
                                           figsize=(6, 3), dpi=300):
    """Save format2-style chart that replaces the forecast line with PVGIS."""
    import matplotlib.dates as mdates
    plt.figure(figsize=figsize, dpi=dpi)
    plt.plot(index, y_true, label='Actual', color='black', linewidth=2.5)
    plt.plot(index, y_pvgis, label='PVGIS P_mapped_Wh', color='dimgray', linewidth=2.5)
    plt.plot(index, y_naive, label='LastValueNaive', color='gray', linewidth=2, linestyle='--')
    plt.title(title or 'Wh PVGIS vs Actual vs LastValueNaive', fontsize=15, pad=12)
    plt.xlabel('Date', fontsize=13)
    plt.ylabel('Wh', fontsize=13)
    plt.grid(alpha=0.4, linestyle=':', linewidth=0.8)
    ax = plt.gca()
    ...
    plt.xticks(fontsize=11, rotation=30)
    plt.yticks(fontsize=11)
    if mase is not None or rmsse is not None:
        metrics_parts = []
        if mase is not None:
            metrics_parts.append(f'MASE={mase:.3f}')
        if rmsse is not None:
        ...
        if metrics_text:
            ax.text(1.03, 0.98, metrics_text, transform=ax.transAxes,
                    fontsize=9, va='top', ha='left', bbox=dict(facecolor='white', alpha=0.9, edgecolor='none'))
    plt.legend(loc='upper left', bbox_to_anchor=(1.01, 0.60), fontsize=10, frameon=False)
    plt.tight_layout(rect=[0, 0, 0.85, 1])
    plt.savefig(plot_path, dpi=dpi, bbox_inches='tight')
    plt.close()
...
def save_forecast_plot_variants(plot_df, out_dir, horizon, mase, rmsse, nMAE, nRMSE, r2, smape, pvgis_metrics=None):
    """Generate all forecast plot variants using the existing plot routines."""
    variants = [
        ('', plot_forecast_comparison, {}, (12, 6), 150),
        ('-format2', plot_forecast_comparison_legacy, dict(mase=mase, rmsse=rmsse, smape=smape), (6, 3), 300),
        ('-format3', plot_forecast_comparison_format3, dict(rmsse=rmsse, nMAE=nMAE, nRMSE=nRMSE, r2=r2, smape=smape), (6, 3), 300),
        ('-format4', plot_forecast_comparison_format4, dict(mase=mase, rmsse=rmsse, nMAE=nMAE, nRMSE=nRMSE, r2=r2, smape=smape), (6, 3), 300),
    ]
    for suffix, plot_func, extra, figsize, dpi in variants:
        plot_path = os.path.join(out_dir, f'forecast_vs_actual_vs_lastvalue_{horizon}d{suffix}.png')
        try:
            plot_func(
                plot_path,
                plot_df.index,
                plot_df['Actual'],
                plot_df['Forecast'],
                plot_df['LastValueNaive'],
                title=f'Wh Forecast vs Actual vs LastValueNaive ({horizon}d)',
                figsize=figsize,
                dpi=dpi,
                **extra,
            )
    ...
    pvgis_plot_path = os.path.join(out_dir, f'forecast_vs_actual_vs_lastvalue_vs_PVGIS_{horizon}d.png')
    print('DEBUG: about to generate PVGIS comparison chart, output=', pvgis_plot_path)
    try:
        plot_forecast_vs_actual_naive_vs_pvgis(
            pvgis_plot_path,
            plot_df,
            PVGIS_DAILY_SCALED,
            title=f'Wh Forecast vs Actual vs LastValueNaive vs PVGIS ({horizon}d)',
            figsize=(12, 6),
            dpi=150,
        )
        print('Chart saved to', pvgis_plot_path)
    except Exception as e:
        print('Failed to generate PVGIS comparison chart:', e)

    pvgis_format2_path = os.path.join(out_dir, f'PVGIS_vs_actual_vs_lastvalue_{horizon}d-format2.png')
    try:
        pvgis_series = load_aligned_pvgis_series(plot_df.index, PVGIS_DAILY_SCALED)
        pvgis_mase = None
        pvgis_rmsse = None
        if pvgis_metrics is not None and isinstance(pvgis_metrics, dict):
            pvgis_mase = pvgis_metrics.get('PVGIS', {}).get('MASE_lag1')
            pvgis_rmsse = pvgis_metrics.get('PVGIS', {}).get('RMSSE')
        plot_forecast_comparison_format2_pvgis(
            pvgis_format2_path,
            plot_df.index,
            plot_df['Actual'],
            pvgis_series,
            plot_df['LastValueNaive'],
            title=f'PVGIS vs Actual vs LastValueNaive ({horizon}d)',
            mase=pvgis_mase,
            rmsse=pvgis_rmsse,
            figsize=(6, 3),
            dpi=300,
        )
        print('Chart saved to', pvgis_format2_path)
    except Exception as e:
        print('Failed to generate PVGIS format2 chart:', e)
            ...
            try:
                ...
                try:
                    ...
                    if FORBID_MODEL_OVERRIDE:
                        allow_arima = handle_model_override(original_list, attempted_list, out_root=out_dir if 'out_dir' in locals() else None, horizon=getattr(model, 'forecast_length', None), action=ON_OVERRIDE_ACTION)
                    if not allow_arima:
                    ...
                    model = fallback_model
                        ...
                        print('Final fallback: 使用 lastValueNaive baseline 作為保險回退，避免整個 run 崩潰')
                        try:
                            y_lastvalue = compute_lastvalue_baseline(train_value_df, test_df)
                            forecast_df = pd.DataFrame({'Wh': y_lastvalue}, index=test_df.index)
...
def generate_forecast_outputs(train_df, test_df, train_value_df, prediction, model, out_dir, horizon):
    ...
    scores, mae, mase, rmsse, smape = compute_forecast_scores(
        y_true,
        y_pred,
        train_df['Wh'].astype(float).values,
        baseline_lastvalue=y_lastvalue,
    ...
    pvgis_metrics = None
    try:
        pvgis_metrics = save_pvgis_vs_naive_metrics(train_df, test_df, out_dir, horizon)
    except Exception as e:
                print('Failed to save PVGIS vs LastValueNaive metrics:', e)

    plot_df = pd.DataFrame(
        {'Actual': y_true, 'Forecast': y_pred, 'NaiveLag1': y_naive, 'LastValueNaive': y_lastvalue},
        index=test_df.index,
    )
    save_forecast_plot_variants(
        plot_df,
        out_dir,
        horizon,
        mase,
...
def load_and_align_tmy(path, target_index, use_columns=None):
    """Load a TMY CSV, detect datetime column, select numeric/use_columns, reindex and fill.

    Returns a DataFrame indexed by target_index with numeric columns only.
    """
    if not os.path.exists(path):
        raise FileNotFoundError(f"TMY file not found: {path}")
    tmy = pd.read_csv(path, low_memory=False)
    # auto-detect common datetime-like column
    date_col = None
    for c in tmy.columns:
        low = c.lower()
        if any(tok in low for tok in ('localtime', 'time', 'date', 'timestamp')):
            date_col = c
            break
    if date_col is None:
        # fallback to first column
        date_col = tmy.columns[0]
    tmy[date_col] = pd.to_datetime(tmy[date_col], errors='coerce')
    tmy = tmy.set_index(date_col).sort_index()

    if use_columns is not None:
        # keep only requested columns that exist
        cols = [c for c in use_columns if c in tmy.columns]
        tmy = tmy[cols]
    else:
        # auto-select numeric cols
        tmy = tmy.select_dtypes(include=[np.number])

    # reindex to the target index and fill conservatively
    try:
        tmy = tmy.reindex(target_index)
    except Exception:
        # if reindex fails, attempt to coerce index to date and then reindex
        try:
            tmy.index = pd.to_datetime(tmy.index, errors='coerce')
            tmy = tmy.reindex(target_index)
        except Exception:
            pass
    tmy = tmy.ffill().bfill()
    ...
    for c in tmy.columns:
        tmy[c] = pd.to_numeric(tmy[c], errors='coerce')
    return tmy


def _merge_fr(*dfs):
    """Merge multiple future_regressor DataFrames by concat on columns; returns None if none provided."""
    dfs = [d for d in dfs if d is not None]
    if not dfs:
        return None
    out = pd.concat(dfs, axis=1)
    # coerce to numeric, drop duplicate columns if names collide
...
def main():
    ...
    try:
        ...
        # TMY (Typical Meteorological Year) options
        parser.add_argument('--enable_tmy')
        parser.add_argument('--tmy_file')
        parser.add_argument('--tmy_columns')
        ...
        # CLI overrides for strict override behavior
        ...
        if getattr(args, 'on_override_action', None) is not None:
            v = args.on_override_action.strip()
            globals()['ON_OVERRIDE_ACTION'] = v
            applied.append('ON_OVERRIDE_ACTION')

        # CLI overrides for TMY
        if getattr(args, 'enable_tmy', None) is not None:
            b = _parse_bool(args.enable_tmy)
            if b is not None:
                globals()['ENABLE_TMY'] = b
                applied.append('ENABLE_TMY')
        if getattr(args, 'tmy_file', None) is not None:
            p = args.tmy_file.strip()
            if not os.path.isabs(p):
                p = os.path.normpath(os.path.join(INPUT_DIR, p))
            globals()['TMY_PATH'] = p
            applied.append('TMY_PATH')
        if getattr(args, 'tmy_columns', None) is not None:
            parsed = _parse_list_or_json(args.tmy_columns)
            if isinstance(parsed, list):
                globals()['TMY_USE_COLUMNS'] = parsed
                applied.append('TMY_USE_COLUMNS')
    ...
    try:
        echo_keys = ['default_max_generations', 'horizons', 'InfiniteLoop',
             ...
             'ENABLE_FUTURE_REGRESSOR', 'ENABLE_FIT_FUTURE_REGRESSOR', 'ENABLE_PREDICT_FUTURE_REGRESSOR', 'random_seed',
               'ENABLE_TMY', 'TMY_PATH', 'TMY_USE_COLUMNS', 'ENABLE_PVGIS_AS_FUTURE_REGRESSOR',
    ...
    # 使用模組頂端常數作為輸入檔案路徑, 支援命令列覆寫 / 兼容多個常用資料目錄
    csv_path = getattr(args, 'input_file', None) or DEFAULT_INPUT

    fallbacks = [
        DEFAULT_INPUT,
        os.path.normpath(os.path.join(INPUT_DIR, 'SolarRecord(260228)_d_forWh_WithCodis.csv')),
        os.path.normpath(os.path.join(os.path.dirname(__file__), '..', 'csv', '2000--202602-d-forWh.csv')),
    ]
    ...
    if not os.path.exists(csv_path):
        ...
        for candidate in fallbacks:
            if os.path.exists(candidate):
                print(f"Found fallback input file: {candidate}")
                csv_path = candidate
                break

    if not os.path.exists(csv_path):
        raise FileNotFoundError(
            f"No input CSV found. Tried: {csv_path}, "
            f"{', '.join(fallbacks)}\n" \
            f"Please create the input file or pass --input_file <path>."
        )

    print('Using input CSV:', csv_path)
        ...
        for idx, horizon in enumerate(horizons):
            ...
            try:
                if isinstance(default_model_list, list):
                    ...
                    if not valid_models:
                        print('AutoTS: no valid models found in default_model_list; would use AutoTS default')
                        if FORBID_MODEL_OVERRIDE:
                            allow = handle_model_override(default_model_list, 'default', out_root=out_dir if 'out_dir' in locals() else None, horizon=horizon, action=ON_OVERRIDE_ACTION)
                            if not allow:
                                print('Model validation would override model_list; skipping this horizon due to FORBID_MODEL_OVERRIDE')
                                continue
                ...
                if 'transformer_list' in msg or 'alias not recognized' in msg:
                    ...
                    try:
                        ...
                        if FORBID_MODEL_OVERRIDE:
                            allow = handle_model_override(default_model_list, 'default', out_root=out_dir if 'out_dir' in locals() else None, horizon=horizon, action=ON_OVERRIDE_ACTION)
                            if not allow:
                                print('Fallback to default model_list blocked by FORBID_MODEL_OVERRIDE; re-raising original error.')
                                raise
            ...
            # --- prepare optional TMY regressors (fit/predict) ---
            fit_fr_candidate = None
            pred_fr_candidate = None
            if ENABLE_TMY:
                try:
                    fit_fr_candidate = load_and_align_tmy(TMY_PATH, train_df.index, use_columns=TMY_USE_COLUMNS)
                    pred_fr_candidate = load_and_align_tmy(TMY_PATH, test_df.index, use_columns=TMY_USE_COLUMNS)
                    print('Loaded TMY regressors:', list(fit_fr_candidate.columns))
                except Exception as e:
                    print('TMY load/align failed, continuing without TMY:', e)
                    fit_fr_candidate = None
                    pred_fr_candidate = None

            try:
                model, train_wide, fit_fr = fit_autots_model(model, train_df, fit_fr_candidate, out_dir, default_num_validations)
            except ValueError as e:
            ...
            pred_fr = pred_fr_candidate if 'pred_fr_candidate' in locals() else None
            if ENABLE_FUTURE_REGRESSOR and ENABLE_PREDICT_FUTURE_REGRESSOR and FUTURE_REGRESSOR_PREDICT:
                try:
                    ...
                    if os.path.exists(p_path):
                        loaded_pred = None
                        try:
                            loaded_pred = pd.read_csv(p_path, parse_dates=['LocalTime'], index_col='LocalTime', low_memory=True)
                        except Exception:
                            loaded_pred = pd.read_csv(p_path, low_memory=True)
                            if 'LocalTime' in loaded_pred.columns:
                                loaded_pred.index = pd.to_datetime(loaded_pred['LocalTime'], errors='coerce')
                                loaded_pred = loaded_pred.drop(columns=['LocalTime'])
                        try:
                            loaded_pred = loaded_pred.reindex(test_df.index)
                        except Exception:
                            pass
                        # Drop target column if accidentally present
                        if 'Wh' in loaded_pred.columns:
                            loaded_pred = loaded_pred.drop(columns=['Wh'])
                        # Automatic cleaning for predict regressors:
                        try:
                            for c in list(loaded_pred.columns):
                                try:
                                    loaded_pred[c] = pd.to_numeric(loaded_pred[c], errors='coerce')
                                except Exception:
                                    pass
                            non_allnan = [c for c in loaded_pred.columns if not loaded_pred[c].isna().all()]
                            if len(non_allnan) != len(loaded_pred.columns):
                                dropped = set(loaded_pred.columns) - set(non_allnan)
                                try:
                                    loaded_pred = loaded_pred[non_allnan]
                                except Exception:
                                    pass
                                print('Dropped all-NaN regressors for predict:', list(dropped))
                            try:
                                loaded_pred = loaded_pred.fillna(method='ffill').fillna(method='bfill').fillna(0)
                            except Exception:
                                loaded_pred = loaded_pred.fillna(0)
                            if loaded_pred.shape[1] == 0:
                                print('No valid future regressors remain after cleaning; skipping file-based predict regressors')
                                loaded_pred = None
                        except Exception as e_clean:
                            print('Warning: failed to clean/preserve future_regressor:', e_clean)

                        # Merge with any existing pred_fr (e.g., from TMY)
                        try:
                            pred_fr = _merge_fr(pred_fr, loaded_pred)
                        except Exception:
                            pred_fr = loaded_pred or pred_fr
            ...
            save_forecast_plot_variants(
                ...
                mase,
                rmsse,
                scores.get('nMAE') if isinstance(scores, dict) else None,
                scores.get('nRMSE') if isinstance(scores, dict) else None,
                scores.get('R2') if isinstance(scores, dict) else None,
                scores.get('SMAPE(%)') if isinstance(scores, dict) else None,
                pvgis_metrics=pvgis_metrics,
            )
            ...
            try:
                ...
                pass
            try:
                save_pvgis_vs_naive_metrics(train_df, test_df, out_dir, horizon)
            except Exception as e:
                print('Failed to save PVGIS vs naive metrics:', e)
            try:
                res_path = os.path.join(out_dir, f'autots_model_results_{horizon}d.json')
                with open(res_path, 'w', encoding='utf-8') as f:
                    json.dump(model.results(), f, ensure_ascii=False, indent=2)
