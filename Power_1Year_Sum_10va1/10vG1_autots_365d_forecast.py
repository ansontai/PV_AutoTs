...
def main(args) -> None:
    ...
    try:
        ...
        try:
            ...
            if "out of bounds for int8" in msg or "overflow" in msg:
                ...
                fallback_model = AutoTS(
                    ...
                    num_validations=0,
                    validation_method=VALIDATION_METHOD,
                    ensemble=["simple", "horizontal"],
                    model_list=MODEL_LIST_FALLBACK_SAFE ,
                    # model_list="default", # fallback 模型不限制在 fast，讓它有機會找到更好的模型
                    no_negatives=NO_NEGATIVES,
                )
        ...
        try:
            annual_agg = compute_forecast_aggregates(out_forecast, freq="A")
