...
#csv_filename = "2000--202602-d.csv"
csv_filename = "2000--202602-d-forWh.csv"
...
# 預測參數
forecast_length = 365  # 預測未來 365 天
frequency = "D"
...
model_kwargs = {
    ...
    "frequency": frequency,
    # 提高準確性：改用較強的 ensemble 與較完整的模型池
    #ensemble": "horizontal",
    "ensemble": "simple",
    # 使用 default 精度通常高於 fast，且較不容易踩到額外深度學習依賴
    #"model_list": "default",
    "model_list": "superfast", 
    "prediction_interval": 0.9,
    # 讓 AutoTS 自動搜尋較多轉換器組合
...
# fallback: 避免部分模型需要 torch 導致整批欄位失敗
fallback_model_kwargs = dict(model_kwargs)
fallback_model_kwargs.update(
    {
        "model_list": "fast",
        "ensemble": "simple",
        "max_generations": 5,
...
def fit_predict_autots(ts_df, model_config, horizon):
    """Fit AutoTS and predict; retry with fallback config on torch-related errors."""
    try:
        model = AutoTS(**model_config)
        model = model.fit(ts_df, date_col="ds", value_col="y")
        prediction = model.predict(forecast_length=horizon)
        return prediction.forecast
    except Exception as exc:
        if "torch" in str(exc).lower() and model_config is not fallback_model_kwargs:
            print("  偵測到 torch 相關錯誤，改用 torch-safe 參數重試")
            model = AutoTS(**fallback_model_kwargs)
            model = model.fit(ts_df, date_col="ds", value_col="y")
            prediction = model.predict(forecast_length=horizon)
            return prediction.forecast
        raise
...
for col in target_cols:
    ...
    if len(ts) > forecast_length + 1:
        ...
        try:
            valid_forecast_df = fit_predict_autots(train_ts, model_kwargs, forecast_length)
    ...
    try:
        forecast = fit_predict_autots(ts, model_kwargs, forecast_length)
    except Exception as exc:
